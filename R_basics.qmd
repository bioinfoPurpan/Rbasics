...
head(genes_data[, 1:3])
head(t(genes_data))
genes_data$sample2 = runif(1000)*10
```
:::
...
colnames(genes_data)[2:4][which.max(colMeans(genes_data[,2:4]))]

rownames(genes_data)[which.min(apply(genes_data[,2:4], 1, median))]

genes_data[order(-rowSums[genes_data[,2:4]]),]
```
:::
...
3. the expression of gene 333 againt gene 666. Surimpose in red triangles the expression of gene 333 against gene 667.
:::
