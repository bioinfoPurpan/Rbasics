...
## Vectors Creation {auto-animate=true}
...
neo[1,1] # print cell in intersection of row 1 and column 1
neo[1,] # print the row 1
neo[,1] # print the column 1
neo[1:2,1:3] # print cells from row 1 to 2 and from row 1 to 3

neo[1:2,1:3] = matrix(rep(1,6),nrow=2) # replace part of the matrix with another matrix
neo
```
:::
...
genes_data <- data.frame(
  name = c("gene_1", "gene_2", "gene_3", "gene_4"),
  sample2 = c(1,10,50,3),
  sample1 = c(20,40,0,3),
  has_peaks = c(TRUE, FALSE, TRUE, TRUE)
)
...
genes_data
genes_data["gene_1", "sample1"]
genes_data[,1:3]
genes_data$sample1

genes_data$sample1 = c(0,0,0,0)
genes_data
```
:::
...
genes_data2 <- data.frame(
  name = c("gene_6", "gene_7", "gene_8", "gene_9"),
  sample2 = c(1,10,50,3)*100,
  sample1 = c(20,40,0,3)*12,
  has_peaks = c(FALSE, FALSE, TRUE, TRUE)
)

rownames(genes_data2)
colnames(genes_data2)
...
::: { .callout appearance="simple" }
You can use arithmetic operations (`+-*/`) only on numeric columns. 
Functions like `apply()` can be used as with a matrix.
:::
...
# Step 1
genes_data[,"sample1"] = genes_data[,"sample1"]*100
genes_data[,3] = genes_data[,3]*200
genes_data[,4] = genes_data[,4]*300

# Step 2
mean_per_col <- colMeans(genes_data[,2:4])
samples <- colnames(genes_data[,2:4])
samples[which.max(mean_per_col)]

# Step 3
rownames(genes_data)[which.min(apply(genes_data[,2:4], 1, median))]
```
:::
...
axis(side=4, at = pretty(range(mat.ge["gene667",])), col="red",col.axis="red")
```
:::
...
## Firstly calculate median for each gene
geneMed = apply(mat.ge,1,median)
geneMed

## Secondly test this median gene list to detect those > 1
test_median = geneMed>=1
test_median
summary(test_median)

## Finally only select genes with median > 1 in the data.frame
mat.ge.filter = mat.ge[which(test_median>=1),]
dim(mat.ge)
dim(mat.ge.filter)
```
:::
